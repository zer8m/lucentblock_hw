//! 가격-시간 우선 원칙(price-time priority) 매칭 엔진.
//!
//! 오더북 = BTreeMap<가격, VecDeque<주문>>
//!   - BTreeMap: 가격이 항상 정렬 -> 최우선 호가를 O(log N)에 찾음
//!   - VecDeque: 같은 가격 안에서는 먼저 들어온 주문이 앞(선착순)

use std::collections::{BTreeMap, HashMap, VecDeque};
use std::sync::mpsc::{channel, Receiver, Sender};
use std::thread;

pub type OrderId = u64;
/// 가격. 부동소수점 오차를 피하려고 항상 정수(최소 호가 단위 기준)로 다룬다.
pub type Price = u64;
/// 수량. 가격과 같은 이유로 정수.
pub type Qty = u64;

/// 주문 방향
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Side {
    Buy,  // 매수
    Sell, // 매도
}

/// 주문 종류
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OrderType {
    Limit,  // 지정가: 못 채우면 오더북에 남아 대기
    Market, // 시장가: 즉시 체결 가능한 만큼만 체결, 나머지는 취소
}

#[derive(Debug, Clone)]
pub struct Order {
    pub id: OrderId,
    pub user_id: u64,
    pub side: Side,
    pub order_type: OrderType,
    /// 시장가 주문에서는 무시된다.
    pub price: Price,
    pub qty: Qty,
    pub remaining_qty: Qty,
    pub timestamp: u64,
}

impl Order {
    pub fn limit(id: OrderId, user_id: u64, side: Side, price: Price, qty: Qty, timestamp: u64) -> Self {
        Self { id, user_id, side, order_type: OrderType::Limit, price, qty, remaining_qty: qty, timestamp }
    }

    pub fn market(id: OrderId, user_id: u64, side: Side, qty: Qty, timestamp: u64) -> Self {
        Self { id, user_id, side, order_type: OrderType::Market, price: 0, qty, remaining_qty: qty, timestamp }
    }
}

/// 체결 영수증. 가격은 항상 오더북에서 기다리던 maker의 가격이다.
/// 유저 id와 taker 방향까지 담아야 잔고 이체(누가 사고 누가 팔았는지)가 가능하다.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Trade {
    pub maker_order_id: OrderId,
    pub taker_order_id: OrderId,
    pub maker_user_id: u64,
    pub taker_user_id: u64,
    /// taker의 방향. Buy면 taker가 매수자, Sell이면 maker가 매수자.
    pub taker_side: Side,
    pub price: Price,
    pub qty: Qty,
}

#[derive(Debug, Default)]
pub struct OrderBook {
    bids: BTreeMap<Price, VecDeque<Order>>,
    asks: BTreeMap<Price, VecDeque<Order>>,
    /// 지금까지의 전체 체결 내역(발생 순서대로).
    trades: Vec<Trade>,
    /// 체결이 날 때마다 이벤트를 내보내는 채널. 블로그의 WAL/브로드캐스트 채널 자리.
    trade_tx: Option<Sender<Trade>>,
}

impl OrderBook {
    pub fn new() -> Self {
        Self::default()
    }

    /// 매수 최우선 호가(가장 비싼 값).
    pub fn best_bid(&self) -> Option<Price> {
        self.bids.keys().next_back().copied()
    }

    /// 매도 최우선 호가(가장 싼 값).
    pub fn best_ask(&self) -> Option<Price> {
        self.asks.keys().next().copied()
    }

    /// 호가창 스냅샷. 매수는 비싼 순, 매도는 싼 순으로 (가격, 잔량).
    pub fn depth(&self, side: Side) -> Vec<(Price, Qty)> {
        let sum = |q: &VecDeque<Order>| -> Qty { q.iter().map(|o| o.remaining_qty).sum() };
        match side {
            Side::Buy => self.bids.iter().rev().map(|(&p, q)| (p, sum(q))).collect(),
            Side::Sell => self.asks.iter().map(|(&p, q)| (p, sum(q))).collect(),
        }
    }

    /// 주문 하나를 처리하고 발생한 체결 목록을 돌려준다.
    ///
    /// 지정가: 반대편에서 조건이 맞는 만큼 체결하고, 남으면 오더북에 등록되어 maker가 된다.
    /// 시장가: 체결 가능한 만큼만 체결하고 남은 수량은 취소(오더북에 등록하지 않음).
    pub fn process_order(&mut self, mut taker: Order) -> Vec<Trade> {
        let mut trades = Vec::new();

        while taker.remaining_qty > 0 {
            // 1단계: 반대편 최우선 호가를 본다.
            let book = if taker.side == Side::Buy { &mut self.asks } else { &mut self.bids };
            let best = match taker.side {
                Side::Buy => book.keys().next().copied(),
                Side::Sell => book.keys().next_back().copied(),
            };
            let Some(price) = best else { break };
            if !crosses(&taker, price) {
                break;
            }

            // 2단계: 그 가격의 대기줄 앞에서부터 체결한다.
            let queue = book.get_mut(&price).expect("best price must exist");
            while taker.remaining_qty > 0 {
                let Some(maker) = queue.front_mut() else { break };
                let qty = taker.remaining_qty.min(maker.remaining_qty);
                taker.remaining_qty -= qty;
                maker.remaining_qty -= qty;
                trades.push(Trade {
                    maker_order_id: maker.id,
                    taker_order_id: taker.id,
                    maker_user_id: maker.user_id,
                    taker_user_id: taker.user_id,
                    taker_side: taker.side,
                    price, // maker 가격으로 체결
                    qty,
                });
                // 3단계: 다 체결된 maker는 줄에서 뺀다.
                if maker.remaining_qty == 0 {
                    queue.pop_front();
                }
            }
            if queue.is_empty() {
                book.remove(&price);
            }
        }

        // 지정가에서 남은 물량은 스스로 maker가 되어 대기.
        if taker.order_type == OrderType::Limit && taker.remaining_qty > 0 {
            let book = if taker.side == Side::Buy { &mut self.bids } else { &mut self.asks };
            book.entry(taker.price).or_default().push_back(taker);
        }

        self.trades.extend_from_slice(&trades);
        if let Some(tx) = &self.trade_tx {
            for t in &trades {
                // 수신자가 먼저 사라져도 매칭 자체는 계속되어야 하므로 에러는 무시.
                let _ = tx.send(*t);
            }
        }
        trades
    }

    /// 체결 이벤트를 받을 채널을 연결한다. WAL 스레드, 브로드캐스트 등 소비자는 반대편에서 수신.
    pub fn set_trade_sender(&mut self, tx: Sender<Trade>) {
        self.trade_tx = Some(tx);
    }

    /// 전체 체결 내역 조회.
    pub fn trades(&self) -> &[Trade] {
        &self.trades
    }

    /// 대기 중인 주문을 오더북에서 빼고 돌려준다. 없으면(이미 체결/취소) None.
    // ponytail: 전체 스캔 O(N). 주문 수가 커지면 id -> (side, price) HashMap 인덱스로 업그레이드.
    pub fn cancel_order(&mut self, id: OrderId) -> Option<Order> {
        for book in [&mut self.bids, &mut self.asks] {
            let found = book
                .iter()
                .find_map(|(&price, queue)| queue.iter().position(|o| o.id == id).map(|pos| (price, pos)));
            if let Some((price, pos)) = found {
                let queue = book.get_mut(&price).expect("price level must exist");
                let order = queue.remove(pos).expect("position must exist");
                if queue.is_empty() {
                    book.remove(&price);
                }
                return Some(order);
            }
        }
        None
    }
}

/// 유저별 잔고(현금·자산). 체결 반영만 담당한다.
///
/// 주문 접수 시 잔고 사전 검증은 하지 않으므로 잔고가 음수로 내려갈 수 있다.
/// 그래서 u64 대신 i64를 쓴다(언더플로 패닉 대신 음수로 남겨 돈이 사라지지 않게).
#[derive(Debug, Default)]
pub struct Accounts {
    cash: HashMap<u64, i64>,
    asset: HashMap<u64, i64>,
}

impl Accounts {
    pub fn new() -> Self {
        Self::default()
    }

    /// 입금. 현금과 자산을 한 번에 넣는다.
    pub fn deposit(&mut self, user_id: u64, cash: i64, asset: i64) {
        *self.cash.entry(user_id).or_default() += cash;
        *self.asset.entry(user_id).or_default() += asset;
    }

    /// 체결 1건 반영: 매수자 현금 -> 매도자, 매도자 자산 -> 매수자.
    pub fn apply_trade(&mut self, t: &Trade) {
        let (buyer, seller) = match t.taker_side {
            Side::Buy => (t.taker_user_id, t.maker_user_id),
            Side::Sell => (t.maker_user_id, t.taker_user_id),
        };
        let cost = (t.price * t.qty) as i64;
        *self.cash.entry(buyer).or_default() -= cost;
        *self.cash.entry(seller).or_default() += cost;
        *self.asset.entry(seller).or_default() -= t.qty as i64;
        *self.asset.entry(buyer).or_default() += t.qty as i64;
    }

    pub fn cash(&self, user_id: u64) -> i64 {
        self.cash.get(&user_id).copied().unwrap_or(0)
    }

    pub fn asset(&self, user_id: u64) -> i64 {
        self.asset.get(&user_id).copied().unwrap_or(0)
    }
}

/// 엔진 스레드에 보내는 명령. 응답이 필요한 명령은 회신 채널(reply)을 실어 보낸다.
pub enum EngineCommand {
    /// 주문 접수. 주문 id는 호출자(서버)가 부여해서 보낸다. 회신: 체결 목록.
    Order {
        order_id: OrderId,
        user_id: u64,
        side: Side,
        order_type: OrderType,
        /// 시장가면 무시된다.
        price: Price,
        qty: Qty,
        reply: Sender<Vec<Trade>>,
    },
    /// 회신: 취소된 주문 id. 없으면 None.
    Cancel { order_id: OrderId, reply: Sender<Option<OrderId>> },
    Deposit { user_id: u64, cash: i64, asset: i64 },
    /// 회신: (매수 호가창, 매도 호가창).
    Book { reply: Sender<(Vec<(Price, Qty)>, Vec<(Price, Qty)>)> },
    Trades { reply: Sender<Vec<Trade>> },
    /// 회신: (현금, 자산).
    Balance { user_id: u64, reply: Sender<(i64, i64)> },
}

/// 블로그 구조 그대로의 싱글스레드 엔진: 오더북과 잔고를 스레드 하나가 독점 소유하고,
/// 채널이 유일한 입구라서 락 없이 동시성 문제가 없다.
///
/// 반환: (명령을 보낼 채널, 체결 이벤트가 나오는 채널 — WAL/브로드캐스트 소비자 자리)
pub fn start_engine() -> (Sender<EngineCommand>, Receiver<Trade>) {
    let (cmd_tx, cmd_rx) = channel();
    let (event_tx, event_rx) = channel();
    thread::spawn(move || engine_loop(cmd_rx, event_tx));
    (cmd_tx, event_rx)
}

fn engine_loop(rx: Receiver<EngineCommand>, event_tx: Sender<Trade>) {
    let mut book = OrderBook::new();
    let mut accounts = Accounts::new();
    book.set_trade_sender(event_tx);
    // 주문 접수 순서가 곧 시간 우선순위이므로 단조 증가 카운터면 충분하다.
    let mut clock: u64 = 1;

    for cmd in rx {
        match cmd {
            EngineCommand::Order { order_id, user_id, side, order_type, price, qty, reply } => {
                let order = match order_type {
                    OrderType::Limit => Order::limit(order_id, user_id, side, price, qty, clock),
                    OrderType::Market => Order::market(order_id, user_id, side, qty, clock),
                };
                clock += 1;
                let trades = book.process_order(order);
                for t in &trades {
                    accounts.apply_trade(t);
                }
                let _ = reply.send(trades);
            }
            EngineCommand::Cancel { order_id, reply } => {
                let _ = reply.send(book.cancel_order(order_id).map(|o| o.id));
            }
            EngineCommand::Deposit { user_id, cash, asset } => accounts.deposit(user_id, cash, asset),
            EngineCommand::Book { reply } => {
                let _ = reply.send((book.depth(Side::Buy), book.depth(Side::Sell)));
            }
            EngineCommand::Trades { reply } => {
                let _ = reply.send(book.trades().to_vec());
            }
            EngineCommand::Balance { user_id, reply } => {
                let _ = reply.send((accounts.cash(user_id), accounts.asset(user_id)));
            }
        }
    }
}

/// taker가 이 가격의 maker와 거래할 수 있는가?
fn crosses(taker: &Order, maker_price: Price) -> bool {
    match (taker.order_type, taker.side) {
        (OrderType::Market, _) => true, // 시장가는 가격을 안 따진다
        (OrderType::Limit, Side::Buy) => taker.price >= maker_price,
        (OrderType::Limit, Side::Sell) => taker.price <= maker_price,
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    /// 블로그 시나리오: A(10000 매수) -> B(9000 매수) -> C(12000 매도) -> D(10000 매도, 체결)
    #[test]
    fn blog_scenario() {
        let mut book = OrderBook::new();
        assert!(book.process_order(Order::limit(1, 1, Side::Buy, 10_000, 1, 1)).is_empty());
        assert!(book.process_order(Order::limit(2, 2, Side::Buy, 9_000, 1, 2)).is_empty());
        assert!(book.process_order(Order::limit(3, 3, Side::Sell, 12_000, 2, 3)).is_empty());
        assert_eq!(book.best_bid(), Some(10_000));
        assert_eq!(book.best_ask(), Some(12_000));

        let trades = book.process_order(Order::limit(4, 4, Side::Sell, 10_000, 1, 4));
        assert_eq!(
            trades,
            vec![Trade {
                maker_order_id: 1,
                taker_order_id: 4,
                maker_user_id: 1,
                taker_user_id: 4,
                taker_side: Side::Sell,
                price: 10_000,
                qty: 1
            }]
        );
        // A는 빠지고 B가 1등, 매도는 C만 남는다.
        assert_eq!(book.depth(Side::Buy), vec![(9_000, 1)]);
        assert_eq!(book.depth(Side::Sell), vec![(12_000, 2)]);
    }

    #[test]
    fn price_priority_then_time_priority() {
        let mut book = OrderBook::new();
        book.process_order(Order::limit(1, 1, Side::Sell, 1_010, 5, 1)); // 비쌈
        book.process_order(Order::limit(2, 2, Side::Sell, 1_000, 5, 2)); // 쌈 = 우선
        book.process_order(Order::limit(3, 3, Side::Sell, 1_000, 5, 3)); // 같은 가격, 나중에 옴

        let trades = book.process_order(Order::limit(4, 4, Side::Buy, 1_010, 12, 4));
        let ids: Vec<_> = trades.iter().map(|t| (t.maker_order_id, t.price, t.qty)).collect();
        assert_eq!(ids, vec![(2, 1_000, 5), (3, 1_000, 5), (1, 1_010, 2)]);
        // taker가 1010까지 냈어도 체결가는 maker 가격이고, 남은 3개는 매수로 대기.
        assert_eq!(book.depth(Side::Sell), vec![(1_010, 3)]);
        assert!(book.depth(Side::Buy).is_empty());
    }

    #[test]
    fn market_order_does_not_rest() {
        let mut book = OrderBook::new();
        book.process_order(Order::limit(1, 1, Side::Sell, 1_000, 2, 1));

        let trades = book.process_order(Order::market(2, 2, Side::Buy, 5, 2));
        assert_eq!(
            trades,
            vec![Trade {
                maker_order_id: 1,
                taker_order_id: 2,
                maker_user_id: 1,
                taker_user_id: 2,
                taker_side: Side::Buy,
                price: 1_000,
                qty: 2
            }]
        );
        // 못 채운 3개는 오더북에 남지 않는다.
        assert!(book.depth(Side::Buy).is_empty());
        assert!(book.depth(Side::Sell).is_empty());
    }

    #[test]
    fn cancel_removes_resting_order() {
        let mut book = OrderBook::new();
        book.process_order(Order::limit(1, 1, Side::Buy, 1_000, 3, 1));
        book.process_order(Order::limit(2, 2, Side::Buy, 1_000, 2, 2));

        let canceled = book.cancel_order(1).expect("주문 1은 대기 중");
        assert_eq!(canceled.id, 1);
        assert_eq!(book.depth(Side::Buy), vec![(1_000, 2)]);
        // 같은 주문을 두 번 취소할 수는 없다.
        assert!(book.cancel_order(1).is_none());

        // 취소된 주문과는 체결되지 않는다: 남아 있는 주문 2와만 체결.
        let trades = book.process_order(Order::market(3, 3, Side::Sell, 5, 3));
        assert_eq!(trades.len(), 1);
        assert_eq!(trades[0].maker_order_id, 2);
        // 다 비면 가격 레벨도 정리된다.
        assert!(book.best_bid().is_none());
    }

    #[test]
    fn trade_history_accumulates() {
        let mut book = OrderBook::new();
        book.process_order(Order::limit(1, 1, Side::Sell, 1_000, 1, 1));
        book.process_order(Order::limit(2, 2, Side::Sell, 1_100, 1, 2));
        book.process_order(Order::market(3, 3, Side::Buy, 2, 3));

        let hist: Vec<_> = book.trades().iter().map(|t| (t.maker_order_id, t.price)).collect();
        assert_eq!(hist, vec![(1, 1_000), (2, 1_100)]);
    }

    #[test]
    fn accounts_transfer_on_trade() {
        let mut book = OrderBook::new();
        let mut accounts = Accounts::new();
        accounts.deposit(1, 0, 5); // 매도자: 자산 5개
        accounts.deposit(2, 10_000, 0); // 매수자: 현금 10,000

        book.process_order(Order::limit(1, 1, Side::Sell, 1_000, 3, 1));
        for t in book.process_order(Order::limit(2, 2, Side::Buy, 1_000, 3, 2)) {
            accounts.apply_trade(&t);
        }

        // 3개 x 1,000원이 이동했다.
        assert_eq!(accounts.cash(1), 3_000);
        assert_eq!(accounts.asset(1), 2);
        assert_eq!(accounts.cash(2), 7_000);
        assert_eq!(accounts.asset(2), 3);
    }

    #[test]
    fn engine_loop_round_trip() {
        let (engine, events) = start_engine();
        let (reply_tx, reply_rx) = channel();

        engine.send(EngineCommand::Deposit { user_id: 1, cash: 0, asset: 5 }).unwrap();
        engine
            .send(EngineCommand::Order {
                order_id: 1,
                user_id: 1,
                side: Side::Sell,
                order_type: OrderType::Limit,
                price: 1_000,
                qty: 5,
                reply: reply_tx.clone(),
            })
            .unwrap();
        assert!(reply_rx.recv().unwrap().is_empty());

        engine
            .send(EngineCommand::Order {
                order_id: 2,
                user_id: 2,
                side: Side::Buy,
                order_type: OrderType::Market,
                price: 0,
                qty: 2,
                reply: reply_tx,
            })
            .unwrap();
        let trades = reply_rx.recv().unwrap();
        assert_eq!((trades[0].price, trades[0].qty), (1_000, 2));

        // 체결 이벤트 채널로도 같은 체결이 나온다.
        assert_eq!(events.recv().unwrap().qty, 2);

        // 잔고 이체까지 엔진 안에서 끝난다(2번 유저는 입금 없이 사서 현금이 음수).
        let (bal_tx, bal_rx) = channel();
        engine.send(EngineCommand::Balance { user_id: 2, reply: bal_tx }).unwrap();
        assert_eq!(bal_rx.recv().unwrap(), (-2_000, 2));
    }

    #[test]
    fn trade_events_are_sent_to_channel() {
        let (tx, rx) = std::sync::mpsc::channel();
        let mut book = OrderBook::new();
        book.set_trade_sender(tx);

        book.process_order(Order::limit(1, 1, Side::Sell, 1_000, 1, 1));
        book.process_order(Order::market(2, 2, Side::Buy, 1, 2));

        let ev = rx.try_recv().expect("체결 이벤트가 와야 한다");
        assert_eq!((ev.maker_order_id, ev.price, ev.qty), (1, 1_000, 1));
        assert!(rx.try_recv().is_err()); // 체결은 한 건뿐이었다
    }

    #[test]
    fn no_cross_when_price_gap() {
        let mut book = OrderBook::new();
        book.process_order(Order::limit(1, 1, Side::Sell, 1_100, 1, 1));
        assert!(book.process_order(Order::limit(2, 2, Side::Buy, 1_000, 1, 2)).is_empty());
        assert_eq!(book.best_bid(), Some(1_000));
        assert_eq!(book.best_ask(), Some(1_100));
    }
}
